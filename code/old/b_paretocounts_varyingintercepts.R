...
rt <- stanc(file="models/b_paretocounts_varyingintercepts.stan")
sm <- stan_model(stanc_ret = rt, verbose=FALSE)      # compile

fit_pareto_neon_varint  <- sampling(sm, data=dat,iter = 100, chains = 1)

# saveRDS(fit_pareto_neon_varint, file = "models/fit_pareto_neon_varint.rds")
fit_pareto_neon_varint  <- readRDS(file = "models/fit_pareto_neon_varint.rds")
# old_varint <- readRDS(file = "models/fit_pareto_neon_varint.rds")
# saveRDS(old_varint, file = "models/sandbox/old_varint.rds")
