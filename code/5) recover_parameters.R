...
hdi = var_reg_summary %>% 
  ...
  mode_hdci(value) %>% 
  select(model_run, name, .lower, .upper) %>% 
  left_join(true_reg_values) %>% 
...
plot_linear_model_bias = var_reg_bias %>%
  ...
  ggplot(aes(x = bias, y = model_run, group = model_run, fill = true_inside_hdi)) + 
  geom_density_ridges() +
  facet_wrap(~greek_name, scales = "free") + 
  geom_vline(xintercept = 0) +
  theme_default() + 
  # scale_fill_viridis_d(option = "F") + 
  scale_fill_grey(start = 0, end = 0.8) +
  labs(y = "Replicate",
       x = "Bias (modeled value minus true value)") +
  guides(fill = "none")

ggview(plot_linear_model_bias, width = 6.5, height = 2.5, units = "in")
