functions{
  real paretocounts_lpdf(real x, real b_exp, real xmin, real xmax){
    if(b_exp != -1)
...
model {
	// likelihood
	for (i in 1:N){
	  x[i] ~ paretocounts(a + sigma_year*alpha_raw_year[year[i]] + sigma_site*alpha_raw_site[site[i]] + beta*mat_s[i], xmin[i], xmax[i]); 
	  }
  ...
  //hyperpriors
  target += exponential_lpdf(sigma_year|5); 
